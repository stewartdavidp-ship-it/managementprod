...
- **Context budget tracking:** Surfaces report `turnDelta` (chars consumed since last MCP call) on every tool call. Server tracks two running totals per session: `serverSentTotal` (auto-incremented from `_responseSize` of each tool response) and `interactionTotal` (accumulated from `turnDelta`). `_contextHealth` returns `compactionEstimate` (serverSentTotal + interactionTotal), `serverSentTotal`, `interactionTotal`, `ceiling` (624K), and `zone` (green <60%, yellow 60-75%, red 75-90%, imminent >90%). Debounced writes flush to Firebase every 30s. Signal `context-estimate-missing` fires for production surfaces (claude-chat, claude-code) that don't provide `turnDelta`.
